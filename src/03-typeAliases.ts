import {
  ...
  function getBuddyListFriends(list: BuddyList): Friend[] {
    return list.members.reduce((friends: Friend[], buddy) => {
      if ('age' in buddy) {
        // Check if the buddy has the 'age' property (indicating it's a Friend)
  ...
  // Test the function
  const friendList: BuddyList = {
    name: 'Friend List',
    members: [colleague1, friends[0], colleague2],
    administrator: colleague1,
  };
  
  console.log(getBuddyListFriends(friendList));